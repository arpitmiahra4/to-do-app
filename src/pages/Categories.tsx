...
const Categories = () => {
  ...
  const handleAddCategory = () => {
    if (name !== "") {
      ...
      // Add the new category to the top of the categories list
      setUser((prevUser) => ({
        ...prevUser,
        categories: [newCategory, ...prevUser.categories],
      }));
  
      // Sort the categories array based on some criteria (e.g., id)
      setUser((prevUser) => ({
        ...prevUser,
        categories: prevUser.categories.sort((a, b) => a.id.localeCompare(b.id)),
      }));
